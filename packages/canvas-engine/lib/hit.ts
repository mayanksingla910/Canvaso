import { CanvasElement, Point } from "@canvaso/shared-types";
...
function hitTestPolyline(point: Point, points: Point[]): boolean {
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const b = points[i + 1];
    if (!a || !b) continue;
    if (distanceToSegment(point, a, b) <= HIT_PADDING + 4) {
      return true;
    }
  }
  return false;
...
function hitTestMultipleBoundingBoxes(
  point: Point,
  els: CanvasElement[],
): boolean {
  const p = HIT_PADDING;
...
export function hitTestRotationHandle(
  ...
  const handleX = cx - (rawHandleY - cy) * sin;
  const handleY = cy + (rawHandleY - cy) * cos;
...
export function hitTestResizeHandle(
  ...
  for (let i = 0; i < handles.length; i++) {
    ...
    if (!handle) continue;
    if (
      Math.hypot(localPoint.x - handle.x, localPoint.y - handle.y) <= hitRadius
    ) {
      return i;
    }
  }
