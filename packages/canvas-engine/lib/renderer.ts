import { CanvasElement, Point, Viewport } from "@canvaso/shared-types";
...
function drawArrow(
  ...
  const secondToLast = el.points[el.points.length - 2];
  const second = el.points[1];

  if (!start || !end || !secondToLast || !second) return;
  ...
  if (el.endArrowhead !== "none") {
    drawArrowhead(ctx, secondToLast, end, el.endArrowhead, el.strokeWidth);
  }
  if (el.startArrowhead !== "none") {
    drawArrowhead(ctx, second, start, el.startArrowhead, el.strokeWidth);
  }
}
