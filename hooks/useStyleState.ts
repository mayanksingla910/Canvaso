...
export function useApplyStyle() {
  ...
  return (key: keyof DefaultStyles, value: unknown) => {
    const numberKeys: (keyof DefaultStyles)[] = [
      "strokeWidth",
      "opacity",
      "cornerRadius",
    ];
    const coerced = numberKeys.includes(key) ? Number(value) : value;

    setDefault(key, coerced);
    useCanvasStore.getState().updateSelected({ [key]: coerced });
  };
}
